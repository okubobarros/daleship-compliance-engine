// Vercel Serverless Function (Node.js) — proxy server-side para GET /dossies/{id}/apontamentos
// (Render). Clone de resumo.js. Fonte dos cards de achado (confiança + citação + impacto) em
// resultado.html.
const https = require("https");

function buscarApontamentos(apiBase, dossieId, token) {
  return new Promise((resolve, reject) => {
    const url = new URL(`${apiBase}/dossies/${encodeURIComponent(dossieId)}/apontamentos`);
...
module.exports = async (req, res) => {
  try {
    ...
    const upstream = await buscarApontamentos(apiBase, dossieId, token);
